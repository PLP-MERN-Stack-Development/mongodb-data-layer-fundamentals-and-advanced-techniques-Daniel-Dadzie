...
const { connectDB, mongoose } = require('PLP_BookStore/connect');
...
//    const books = await Book.find({ published_year: { $gt: 1855 } });
//    console.log("Books published after 1855:", books);

// find books by a specific author
    // const books = await Book.find({ author: 'Paulo Coelho' });
...
// use projection to return only the title, author, and price fields in your queries
    // const books = await Book.find({}, { title: 1, author: 1, price: 1  });
    // console.log("Books with only title, author, and price fields:", books);

// implement sorting to display books by price (both ascending and descending)
    //  const booksAsc = await Book.find({}).sort({ price: 1 });
    //  console.log("Books sorted by price (ascending):", booksAsc);

    //  const booksDesc = await Book.find({}).sort({ price: -1 });
